...
import { handleAnemoCommand } from './commands'
import { AnemoLogger, PluginLogger } from './logger'
import { AnemoPluginError, loadPlugins } from './plugin'
...
/** 监听上线事件，初始化 Anemo */
export async function onlineHandler(this: Client, anemoConf: AnemoConf) {
  ...
  this.on('message', (event) => {
    ...
    handleAnemoCommand(event, this, anemoConf)
  })
  ...
  // 检索并加载插件
  const { all, cnt, npm, local } = await loadPlugins(this, anemoConf)

  info(colors.cyan(`共检索到 ${all} 个插件 (${local} 个本地，${npm} 个 npm), 启用 ${cnt} 个`))

  // 初始化完成
  AnemoLogger.info(colors.gray('框架初始化完成'))
  AnemoLogger.info(colors.gray('开始处理消息...'))

  // 上线通知，通知 Bot 主管理

  if (!anemoConf.admins[0]) {
    error(colors.red('主管理员必须添加 Bot 为好友，否则无法正常控制 Bot 和发送消息通知'))
  } else {
    const mainAdmin = this.pickFriend(anemoConf.admins[0])
