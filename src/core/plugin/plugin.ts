...
export interface AnemoPluginConf {
  debug?: boolean
  enableGroups?: number[]
  enableFriends?: number[]
}

export class AnemoPlugin extends EventEmitter {
  /** 插件名称 */
  public name: string
  /** 插件版本 */
   ...
   */
  constructor(name: string, version: string, conf?: AnemoPluginConf) {
    super()
    ...
    this.debug('create AnemoPlugin instance')
  ...
  /**
   * 抛出一个 Anemo 插件标准错误，会被框架捕获
   *
   * @param {string} message 错误信息
   */
  throwPluginError(message: string) {
    throw new AnemoPluginError(this.name, message)
  }
  ...
  /**
   * **插件请勿调用**，Anemo 框架调用此函数启用插件
   * @param {Client} bot oicq Client 实例
   * @param {AdminArray} admins 框架管理员列表
   * @return {Promise<AnemoPlugin>} 插件实例
   */
  async mountAnemoClient(bot: Client, admins: AdminArray): Promise<AnemoPlugin> {
    this.debug('mountAnemoClient')
    ...
    // 监听框架管理变动
    bot.on('anemo.admins', this.adminChangeHandler)
  ...
  /**
   * **插件请勿调用**，Anemo 框架调用此函数禁用插件
   * @param {Client} bot oicq Client 实例
   * @param {AdminArray} admins 框架管理员列表
   */
  async unmountAnemoClient(bot: Client, admins: AdminArray) {
    this.debug('unmountAnemoClient')

    // 取消监听框架管理变动
    bot.off('anemo.admins', this.adminChangeHandler)
 ...
 * Anemo 插件类
 */
export interface AnemoPlugin extends EventEmitter {
  /** 监听 oicq 标准事件以及 Anemo 标准事件 */
