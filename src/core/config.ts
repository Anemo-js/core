...
export const anemoConf = {} as AnemoConf

/** 保存 Anemo 框架配置到配置文件`bot.json` */
export const saveAnemoConf = (_plugins?: Map<string, AnemoPlugin>) => {
  try {
    anemoConf.plugins = [...(_plugins ?? plugins).keys()]

    writeJsonSync(ConfigPath, anemoConf, { encoding: 'utf-8', spaces: 2 })
    return true
  } catch (e) {
