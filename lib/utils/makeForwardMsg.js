...
async function makeForwardMsg(msglist, title = '转发的聊天记录', desc = '', footer = '', dm = true) {
    ...
    for (const fake of msglist) {
        ...
        if (cnt < 4) {
            cnt++;
            preview += `<title color="#777777" size="26">${desc || `${(0, common_1.escapeXml)(nickname)}: ${(0, common_1.escapeXml)(maker.brief.slice(0, 50))}`}</title>`;
        }
        nodes.push({
            1: {
                1: fake.user_id,
...
<msg brief="[聊天记录]" m_fileName="${(0, common_1.uuid)().toUpperCase()}" action="viewMultiMsg" tSum="${nodes.length}" flag="3" m_resid="${resid}" serviceID="35" m_fileSize="${compressed.length}"><item layout="1"><title color="#000000" size="34"> ${title} </title>${preview}<hr></hr><summary color="#808080" size="26"> ${footer || `查看 ${nodes.length} 条转发消息`} </summary></item><source name="聊天记录"></source></msg>`;
    const res = {
        type: 'xml',
        data: xml,
        id: 35,
