...
async function deviceHandler(device_mode, event) {
    const info = (msg, ...args) => {
        ...
        logger_1.AnemoLogger.warn(msg, ...args);
    };
    if (device_mode === 'sms') {
        info(`需要验证设备锁，按 \`Enter\` 键发送短信验证码到手机号 ${utils_1.colors.cyan(event.phone)} 进行验证`);
        process.stdin.once('data', async () => {
            this.sendSmsCode();
            info(`短信验证码已发送至手机号 ${utils_1.colors.cyan(event.phone)}，输入后按 \`Enter\` 键继续`);
            const { sms } = await (0, prompts_1.default)({
                type: 'number',
                name: 'sms',
                max: 999999,
                validate: (sms) => (!sms ? '短信验证码不为空' : true),
                message: `请输入短信验证码（${utils_1.colors.cyan(event.phone)}）`
            });
