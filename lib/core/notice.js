...
〓 消息内容 〓\n`.trimStart();
    ...
    bot.on('request.friend.add', async (event) => {
        ...
        if (!config_1.anemoConf.notice.enable || !friend.request.enable)
            return;
...
`.trim();
    ...
    bot.on('notice.friend.increase', async (event) => {
        if (!config_1.anemoConf.notice.enable || !friend.increase)
            return;
...
`.trim();
    ...
    bot.on('notice.friend.decrease', async (event) => {
        if (!config_1.anemoConf.notice.enable || !friend.decrease)
            return;
        const { nickname, user_id } = event;
        const avatar = oicq_1.segment.image((0, utils_1.getQQAvatarLink)(user_id, 100));
        const msg = `
...
`.trim();
    ...
    bot.on('notice.group.increase', async (event) => {
        if (!config_1.anemoConf.notice.enable || !group.increase)
            return;
...
`.trim();
    ...
    bot.on('notice.group.decrease', async (event) => {
        if (!config_1.anemoConf.notice.enable || !group.decrease)
            return;
...
`.trim();
    ...
    bot.on('notice.group.admin', async (event) => {
        if (!config_1.anemoConf.notice.enable || !group.admin)
            return;
